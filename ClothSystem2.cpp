	...
	for (int row = 0; row < numParticles; row++) {
		for (int col = 0; col < numParticles; col++) {
			...
			if (isVerlet) {
				...
				m_vVecState.push_back(Vector3f(0.0f, (float)-1.0f*row*seperation, (float)col*seperation));
				m_vVecState.push_back(Vector3f(0.0f, 0.0f, 0.0f));
				particles.push_back(Vector3f((float)col*seperation, (float)-1.0f*row*seperation, 0.0f));
			}
		}
	}
